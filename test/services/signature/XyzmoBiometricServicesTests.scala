...
class XyzmoBiometricServicesTests extends UnitFlatSpec with ShouldMatchers {
  ...
  "getSignatureDataContainerFromJSON" should "translate JSON signature to Xyzmo SignatureDataContainer" in {
    ...
    val sdc = xyzmo.getSignatureDataContainerFromJSON(signatureStr)
  ...
  it should "test xyzmo end-to-end" in {
    ...
    val xyzmoDeleteUser: XyzmoDeleteUser = xyzmo.deleteUser(enrollmentBatchId = 0, userId = userId)
    ...
    val xyzmoAddUser: XyzmoAddUser = xyzmo.addUser(enrollmentBatchId = 0, userId = userId)
    ...
    val xyzmoAddProfile: XyzmoAddProfile = xyzmo.addProfile(enrollmentBatchId = 0, userId = userId)
    ...
    val xyzmoEnrollUser: XyzmoEnrollDynamicProfile = xyzmo.enrollUser(enrollmentBatchId = 0, userId = userId, List(signature1, signature2, signature3, signature4, signature5, signature6))
    ...
    val xyzmoVerifyUser_Match: XyzmoVerifyUser = xyzmo.verifyUser(egraphId = 0, userId = userId, signature7)
    ...
    val xyzmoVerifyUser_NoMatch: XyzmoVerifyUser = xyzmo.verifyUser(egraphId = 0, userId = userId, signature_nomatch)
