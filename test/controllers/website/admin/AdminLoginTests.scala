...
class AdminLoginTests extends AdminFunctionalTest with CleanDatabaseAfterEachTest {
  ...
  def testAdminLoginProtectsAdminConsole() {
    ...
    assertStatus(403, GET(Utils.lookupUrl("WebsiteControllers.getCelebrities").url))
    assertStatus(403, GET(Utils.lookupUrl("WebsiteControllers.getCreateCelebrity").url))
    assertStatus(403, GET(Utils.lookupUrl("WebsiteControllers.getEgraphs").url))
    assertStatus(403, GET(Utils.lookupUrl("WebsiteControllers.getCelebrityProducts", celebrityIdMap).url))
    assertStatus(403, GET(Utils.lookupUrl("WebsiteControllers.getCreateCelebrityProduct", celebrityIdMap).url))
    assertStatus(403, GET(Utils.lookupUrl("WebsiteControllers.getCelebrityEgraphs", celebrityIdMap).url))
    assertStatus(403, GET(Utils.lookupUrl("WebsiteControllers.getCelebrityOrders", celebrityIdMap).url))
