...
//  }
  ...
  "GalleryOrderFactory" should "create FulfilledEgraphViewModels from orders" in  {
    ...
    val egraph = order.newEgraph.withAsset s(TestConstants.shortWritingStr, Some(TestConstants.shortWritingStr), TestConstants.fakeAudio).
      withEgraphState(EgraphState.PassedBiometrics).approve(admin).save()
