...
public class Plugin extends PlayPlugin {
    ...
    private void handleResponse(VirtualFile file, Request request, Response response) {        
        final String etag = playLessEngine.getEtag(file.getRealFile());

        play.mvc.Http.Header ifNoneMatch = request.headers.get("if-none-match");

        // Log information about server and client etags
        if (ifNoneMatch != null) {
            play.Logger.info("LESS: etag: server=" + etag
                + ", client=" + ifNoneMatch.value());
        } else {
            play.Logger.info("LESS: No client if-none-match header provided");
        }

        // Branch based on whether the client's resource is stale
        if (ifNoneMatch != null && etag != null && etag.equals(ifNoneMatch.value())) {
            handleNotModified(request, response, etag);
        } else {            
            handleOk(request, response, file, etag);
        }
    }
