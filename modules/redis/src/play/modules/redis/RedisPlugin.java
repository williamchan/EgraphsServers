...
public class RedisPlugin extends PlayPlugin {
	...
	public void onApplicationStart() {
    	if (Play.configuration.containsKey("redis.url")) {
            ...
            RedisConnectionManager.shardedConnectionPool = new ShardedJedisPool(makeJedisPoolConfig(), shards, ShardedJedis.DEFAULT_KEY_TAG_PATTERN);
            createdRedis = true;
    	} else {
    ...
    /**
     * Added this method to make sure we never get bogus Jedis
     * instances.
     */
    private static JedisPoolConfig makeJedisPoolConfig() {
        JedisPoolConfig poolConfig = new JedisPoolConfig();

        play.Logger.info("[RedisPlugin] Configuring JedisPool to test instances for validity before adding them.");
        poolConfig.setTestOnBorrow(true);
        
        play.Logger.info("[RedisPlugin] Setting unlimited active and idle connections.");
    ...
    private static class RedisConnectionInfo {
    	...
    	JedisPool getConnectionPool() {
    		if (password == null) {
    			return new JedisPool(makeJedisPoolConfig(), host, port, timeout);
    		}
    		
    		return new JedisPool(makeJedisPoolConfig(), host, port, timeout, password);
    	}
